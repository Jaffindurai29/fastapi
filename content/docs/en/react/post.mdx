---
title: "2. POST"
description: The add form.
---

<GithubLink path="example/5/a" label="Backend on GitHub" />{" "}
<GithubLink path="example/5/react" label="Frontend on GitHub" />

## Backend

```python title="example/5/a/main.py"
class ItemRequest(BaseModel):
    value: str


@app.post("/items", status_code=status.HTTP_201_CREATED)
def create_item(item: ItemRequest):
    data.append(item.value)
    return {"index": len(data) - 1, "value": item.value}
```

## Line by line

1. `status_code=status.HTTP_201_CREATED` on the decorator — `POST`
   routes default to `200`; `201` is the more correct code for "a new
   resource was created."
2. `data.append(item.value)` adds the new value onto the end of the
   shared array.
3. `return {"index": len(data) - 1, "value": item.value}` tells the
   caller exactly where the new item landed, so a follow-up GET, PUT,
   or DELETE knows which index to use.

## Frontend

## Adding a new item

```jsx
async function addItem(event) {
  event.preventDefault();
  if (value.trim() === "") return;

  try {
    await axios.post(`${API_URL}/items`, { value: value });
  } catch (err) {
    setError(errorMessage("add item", err));
    return;
  }

  setValue("");
  await loadItems();
}
```

`event.preventDefault()` stops the browser's default "reload the page"
behavior on form submit. After a successful `POST`, it clears the input
and reloads the list — the new item comes from the server's response to
`loadItems()`, not from guessing what index it landed at.

```jsx
<form onSubmit={addItem}>
  <input value={value} onChange={(e) => setValue(e.target.value)} />
  <button type="submit">Add</button>
</form>
```

## Line by line

1. `event.preventDefault()` stops the browser's default full-page
   reload on form submit.
2. `if (value.trim() === "") return;` skips the request entirely for
   an empty or whitespace-only input.
3. `axios.post(url, { value })` sends `{ value }` as JSON. You pass a
   plain object; axios runs `JSON.stringify` and sets the
   `Content-Type: application/json` header for you. If the server
   rejects it, axios throws and the `catch` shows the error and
   `return`s, so the input isn't cleared.
4. On success, `setValue("")` clears the input, then `loadItems()`
   refreshes the list — the new item comes from the server's response,
   not a guessed index.
5. `<form onSubmit={addItem}>` wires the whole thing to the Add button
   and the Enter key.

Next: [PUT](/en/docs/react/put) — the "Save (PUT)" edit button.

## Code so far

**Backend** — `example/5/a/main.py`, up to POST:

```python title="example/5/a/main.py"
from fastapi import FastAPI, HTTPException, status
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel

app = FastAPI()

app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173", "http://127.0.0.1:5173"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

data = ["first", "second"]


class ItemRequest(BaseModel):
    value: str


@app.get("/items")
def list_items():
    return data


@app.get("/items/{item_index}")
def get_item(item_index: int):
    if item_index < 0 or item_index >= len(data):
        raise HTTPException(status_code=404, detail="Item not found")
    return {"index": item_index, "value": data[item_index]}


@app.post("/items", status_code=status.HTTP_201_CREATED)
def create_item(item: ItemRequest):
    data.append(item.value)
    return {"index": len(data) - 1, "value": item.value}
```

**Frontend** — `example/5/react/src/App.jsx`, up to POST:

```jsx title="example/5/react/src/App.jsx"
import { useEffect, useState } from "react";
import axios from "axios";

const API_URL = "http://127.0.0.1:8000";

function errorMessage(action, err) {
  const status = err.response?.status;
  return status ? `Failed to ${action} (status ${status})` : `Failed to ${action}: ${err.message}`;
}

export default function App() {
  const [items, setItems] = useState([]);
  const [value, setValue] = useState("");
  const [reached, setReached] = useState(null);
  const [error, setError] = useState(null);

  useEffect(() => {
    loadItems();
  }, []);

  async function loadItems() {
    try {
      const response = await axios.get(`${API_URL}/items`);
      setItems(response.data);
      setError(null);
    } catch (err) {
      setError(errorMessage("load items", err));
    }
  }

  async function addItem(event) {
    event.preventDefault();
    if (value.trim() === "") return;

    try {
      await axios.post(`${API_URL}/items`, { value: value });
    } catch (err) {
      setError(errorMessage("add item", err));
      return;
    }

    setValue("");
    await loadItems();
  }

  async function reachItem(index) {
    try {
      const response = await axios.get(`${API_URL}/items/${index}`);
      setReached(response.data);
      setError(null);
    } catch (err) {
      setError(errorMessage(`reach index ${index}`, err));
      setReached(null);
    }
  }

  return (
    <main style={{ maxWidth: 560, margin: "3rem auto", fontFamily: "sans-serif" }}>
      <h1>Array Store</h1>

      {error && <p style={{ color: "crimson" }}>{error}</p>}

      <form onSubmit={addItem} style={{ display: "flex", gap: "0.5rem" }}>
        <input
          value={value}
          onChange={(event) => setValue(event.target.value)}
          placeholder="Add a value"
          style={{ flex: 1, padding: "0.5rem" }}
        />
        <button type="submit">Add</button>
      </form>

      <ul style={{ listStyle: "none", padding: 0, marginTop: "1.5rem" }}>
        {items.map((item, index) => (
          <li
            key={index}
            style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.25rem 0" }}
          >
            <span style={{ color: "#888", width: "1.5rem" }}>{index}</span>
            <span style={{ flex: 1 }}>{item}</span>
            <button onClick={() => reachItem(index)}>Reach</button>
          </li>
        ))}
      </ul>

      {reached && (
        <p style={{ marginTop: "1rem" }}>
          Reached index <strong>{reached.index}</strong>:{" "}
          <strong>{reached.value}</strong>
        </p>
      )}
    </main>
  );
}
```

Still no edit/delete UI — those come next.
