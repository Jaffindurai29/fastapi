...
2. **`ItemPatch.value` is optional** — unlike PUT's `ItemRequest`,
...
1. Same relationship as the array version: one shared `saveEdit`
   function, one string argument deciding the HTTP method.
2. The only change versus the array version is `item.id` replacing
