...
Indha frontend-la ella request-um browser-oda built-in `fetch()`-ku
pathila [axios](https://axios-http.com) moolama dhan pogudhu:
...
| | `fetch()` | `axios` |
|---|---|---|
| Response body | `await response.json()` | already parse aagi `response.data`-la irukkum |
| JSON anuppa | `JSON.stringify(body)` + `Content-Type` header | object-ah pass pannunga, rendaiyum axios pannidum |
| 404 / 500 | normal-a resolve aagum, `response.ok` check pannanum | **throw** pannum — `catch`-la handle aagum |

Kadaisi row-naala dhan `errorMessage` irukku: server error-oda answer
panna, axios status-ah `err.response.status`-la vaikkum. Server-ah
reach-e panna mudiyala na (backend odala, CORS block), `err.response`
irukkadhu, so `err.message`-ku fall back aagum. axios `package.json`-la
irukku, so keezha irukra `npm install` adha install pannidum.
